...
function converteNegativo(){
    ...
    var fs = require('fs');
    var arquivo = fs.createReadStream(path.join(__dirname, './dicionarios') + '/stopwords-portugues.txt', 'utf8');
    ...
    arquivo.on('data', function(chunk) {
        ...
        for (var i = 0; i < dados.length; i++){
            str = dados[i].replace(/\s+/g, '');
            palavras.palavras.push({"palavra": str});
        }

        fs.writeFile('./dicionarios/stopwords-pt.json', JSON.stringify(palavras, null, 4), (err) => {  
            if (err) throw err;
